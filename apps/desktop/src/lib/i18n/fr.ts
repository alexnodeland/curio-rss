import type { MessageKey } from './index';

/**
 * French — complete catalog, following French typographic spacing (a space
 * before : ; ! ?). Every key in {@link ./en} is translated; typed `Partial`
 * only so a future English-only key falls back gracefully. Proper nouns
 * (Curio, OPML, YouTube, Reddit, Google) stay English.
 */
export const fr: Partial<Record<MessageKey, string>> = {
    'app.title': 'Curio',
    'app.tagline': 'Vos lectures, dans vos notes',
    'app.loading': 'Chargement…',
    'app.error.internal': 'Une erreur est survenue. Réessayer peut aider.',
    'a11y.skipToContent': 'Aller au contenu',
    'shell.feeds.loading': 'Chargement des abonnements…',
    'shell.feeds.empty': 'Aucun abonnement pour l’instant',
    'emptyState.hint':
        'Abonnez-vous à un flux pour commencer à lire, ou importez vos abonnements depuis un autre lecteur.',
    'emptyState.addFeed': 'Ajouter votre premier flux',
    'emptyState.importOpml': 'Importer OPML…',
    'shell.feeds.count': '{count, plural, one {# flux} other {# flux}}',
    'shell.unread.count': '{count, plural, one {# non lu} other {# non lus}}',
    'view.all': 'Tous les articles',
    'view.starred': 'Favoris',
    'view.readLater': 'À lire plus tard',
    'view.archived': 'Archivés',
    'view.feeds': 'Flux',
    'sidebar.label': 'Abonnements',
    'sidebar.views': 'Vues',
    'sidebar.feeds': 'Flux',
    'folder.toggle': 'Afficher / masquer {name}',
    'folder.defaultName': 'Nouveau dossier',
    'folder.menu.newFolder': 'Nouveau dossier',
    'folder.menu.newSubfolder': 'Nouveau sous-dossier',
    'folder.menu.rename': 'Renommer',
    'folder.menu.markRead': 'Marquer le dossier comme lu',
    'folder.menu.delete': 'Supprimer le dossier',
    'folder.deleted': 'Supprimé « {name} »',
    'folder.renameCollision': 'Un dossier nommé « {name} » existe déjà ici',
    'folder.empty': 'Dossier vide',
    'feed.menu.openSite': 'Ouvrir le site',
    'feed.menu.refresh': 'Actualiser',
    'feed.menu.rename': 'Renommer',
    'feed.menu.markAllRead': 'Tout marquer comme lu',
    'feed.menu.moveToFolder': 'Déplacer vers un dossier',
    'feed.menu.newFolder': 'Nouveau dossier…',
    'feed.menu.ungroup': 'Retirer du dossier',
    'feed.menu.edit': 'Modifier le flux…',
    'feed.menu.copyUrl': 'Copier l’URL du flux',
    'feed.menu.pause': 'Mettre en pause',
    'feed.menu.resume': 'Reprendre',
    'feed.menu.unsubscribe': 'Se désabonner',
    'feed.unsubscribed': 'Désabonné de « {name} »',
    'sidebar.urlCopied': 'URL du flux copiée',
    'list.label': 'Articles',
    'list.loading': 'Chargement des articles…',
    'list.empty': 'Aucun article ici',
    'list.view.home': 'Vue d’accueil',
    'list.view.rows': 'Vue liste',
    'list.filter.unreadOnly': 'Afficher uniquement les non lus',
    'list.filter.all': 'Afficher tous les articles',
    'list.row.unread': 'Non lu',
    'list.row.starred': 'Favori',
    'reader.empty': 'Sélectionnez un article à lire',
    'reader.imagesOff': 'Les images sont désactivées dans cet article.',
    'reader.imagesOff.enable': 'Activer',
    'reader.missing': 'Cet article n’existe plus',
    'reader.toolbar': 'Actions sur l’article',
    'reader.action.markRead': 'Marquer comme lu',
    'reader.action.markUnread': 'Marquer comme non lu',
    'reader.action.star': 'Ajouter aux favoris',
    'reader.action.unstar': 'Retirer des favoris',
    'reader.action.readLater': 'À lire plus tard',
    'reader.action.readLaterRemove': 'Retirer de la liste à lire',
    'reader.action.archive': 'Archiver',
    'reader.action.unarchive': 'Désarchiver',
    'reader.action.openInBrowser': 'Ouvrir dans le navigateur',
    'reader.action.loadFull': 'Charger l’article complet',
    'reader.action.promote': 'Enregistrer dans les notes',
    'reader.action.typography': 'Typographie',
    'reader.meta.words': '{count, plural, one {# mot} other {# mots}}',
    'reddit.postedBy': 'Publié par u/{author}',
    'reader.youtube.play': 'Lire {title}',
    'reader.youtube.hint':
        'Cliquez pour charger — rien ne se charge tant que vous ne le faites pas',
    'reader.youtube.badge': 'YouTube',
    'reader.youtube.watch': 'Regarder sur YouTube',
    'reader.reddit.openThread': 'Voir les commentaires',
    'reader.reddit.loadFull': 'Charger la publication complète',
    'reader.loadingFull': 'Chargement…',
    'reader.reddit.openLink': 'Ouvrir le lien',
    'typography.title': 'Typographie',
    'typography.font': 'Police',
    'typography.size': 'Taille',
    'typography.lineHeight': 'Interligne',
    'typography.measure': 'Largeur',
    'typography.spacing': 'Espacement',
    'typography.align': 'Alignement',
    'typography.align.left': 'Gauche',
    'typography.align.justify': 'Justifié',
    'typography.weight': 'Graisse',
    'typography.letterSpacing': 'Interlettrage',
    'typography.hyphens': 'Césure',
    'typography.readingTheme': 'Thème de lecture',
    'typography.readingTheme.default': 'Comme l’app',
    'typography.readingTheme.sepia': 'Sépia',
    'typography.readingTheme.paper': 'Papier',
    'typography.reset': 'Réinitialiser',
    'typography.preview': 'Aperçu',
    'typography.preview.heading': 'La forme d’une lecture',
    'typography.preview.body':
        'Une bonne typographie s’efface : elle porte les mots sans attirer l’attention sur elle-même. Ajustez la taille, l’interligne et la largeur jusqu’à ce qu’un long article se lise sans effort.',
    'typography.preview.quote':
        'La typographie est un bel ensemble de lettres, non un ensemble de belles lettres.',
    'typography.preview.listIntro': 'La typographie façonne la lecture de plusieurs manières :',
    'typography.preview.item1':
        'Rythme — un espacement régulier entre les lignes et les paragraphes.',
    'typography.preview.item2': 'Mesure — une longueur de ligne que l’œil suit sans effort.',
    'typography.preview.item3': 'Contraste — une graisse et une couleur qui ne fatiguent jamais.',
    'typography.preview.inlineLead': 'Les éléments en ligne s’intègrent aussi au flux, comme',
    'typography.preview.linkText': 'un lien',
    'tags.label': 'Étiquettes',
    'tags.placeholder': 'Ajouter une étiquette…',
    'tags.add': 'Ajouter une étiquette',
    'tags.remove': 'Supprimer l’étiquette {tag}',
    'tags.empty': 'Aucune étiquette pour l’instant',
    'search.label': 'Rechercher',
    'search.placeholder': 'Rechercher des articles',
    'search.clear': 'Effacer la recherche',
    'search.loading': 'Recherche…',
    'search.empty': 'Aucun résultat',
    'search.results': '{count, plural, one {# résultat} other {# résultats}}',
    'destinations.title': 'Destinations',
    'destinations.open': 'Destinations',
    'destinations.empty':
        'Aucune destination pour l’instant — ajoutez-en une pour enregistrer des articles dans vos notes.',
    'destinations.loading': 'Chargement des destinations…',
    'destinations.name': 'Nom',
    'destinations.namePlaceholder': 'p. ex. notes',
    'destinations.chooseFolder': 'Choisir un dossier…',
    'destinations.add': 'Ajouter une destination',
    'destinations.remove': 'Supprimer {name}',
    'destinations.default': 'Par défaut',
    'destinations.makeDefault': 'Définir par défaut',
    'destinations.exportAll': 'Tout exporter',
    'destinations.exportAll.running': 'Export en cours…',
    'destinations.exportAll.done': '{written} note(s) exportée(s) ; {unchanged} déjà à jour',
    'destinations.needNameAndFolder': 'Saisissez d’abord un nom et choisissez un dossier.',
    'toast.promote.saved': 'Enregistré dans {name}',
    'toast.promote.unchanged': 'Déjà à jour dans {name}',
    'toast.markAllRead':
        '{count, plural, one {# article marqué comme lu} other {# articles marqués comme lus}}',
    'nav.noMoreUnread': 'Plus aucun non lu',
    'help.title': 'Raccourcis clavier',
    'help.close': 'Fermer',
    'help.chord.then': 'puis',
    'help.footer':
        'Clic droit sur les flux et articles pour plus d’actions. Les commandes comme ⌘N et ⌘R sont dans la barre de menus.',
    'modal.close': 'Fermer',
    'toolbar.addFeed': 'Ajouter un flux',
    'toolbar.refreshAll': 'Actualiser tous les flux',
    'toolbar.settings': 'Paramètres',
    'addFeed.title': 'Ajouter un flux',
    'addFeed.urlLabel': 'URL du flux ou du site',
    'addFeed.urlPlaceholder': 'https://example.com',
    'addFeed.find': 'Rechercher des flux',
    'addFeed.finding': 'Recherche…',
    'addFeed.tagsLabel': 'Étiquettes (facultatif)',
    'addFeed.tagsPlaceholder': 'séparées, par des virgules',
    'addFeed.add': 'S’abonner',
    'addFeed.adding': 'Abonnement…',
    'addFeed.candidates': 'Flux trouvés',
    'addFeed.noneFound': 'Aucun flux trouvé sur cette page — essayez l’URL directe du flux.',
    'addFeed.useUrl': 'Utiliser cette URL directement',
    'addFeed.already': 'Vous êtes déjà abonné à ce flux.',
    'addFeed.added': 'Abonné à {title}',
    'addFeed.saveLink': 'Enregistrer pour plus tard',
    'addFeed.savingLink': 'Enregistrement…',
    'addFeed.linkSaved': '« {title} » enregistré pour plus tard',
    'addFeed.linkSavedBare': 'Lien enregistré — la page n’a pas pu être récupérée',
    'addFeed.needUrl': 'Saisissez d’abord une URL.',
    'addFeed.favicon.alt': 'Icône du site',
    'addFeed.favicon.allowRemote':
        'Utiliser le service de favicons de Google quand un site n’en a pas',
    'addFeed.favicon.allowRemoteHint':
        'Désactivé par défaut — l’activer indique à Google quels sites vous prévisualisez.',
    'presets.title': 'Sources populaires',
    'presets.detected': 'Flux détecté :',
    'presets.reddit': 'Reddit',
    'presets.youtube': 'YouTube',
    'presets.mastodon': 'Mastodon',
    'presets.hn': 'Hacker News',
    'presets.hn.frontpage': 'À la une',
    'presets.hn.newest': 'Plus récents',
    'presets.hn.ask': 'Ask HN',
    'presets.hn.show': 'Show HN',
    'presets.hn.best': 'Meilleurs commentaires',

    'settings.title': 'Paramètres',
    'settings.tablist.label': 'Sections des paramètres',
    'settings.section.general': 'Général',
    'settings.section.mediaPrivacy': 'Médias et confidentialité',
    'settings.section.notifications': 'Notifications',
    'settings.notify.enabled': 'Activer les notifications',
    'settings.notify.enabled.hint':
        'Affiche une notification de bureau lorsqu’une actualisation en arrière-plan trouve de nouveaux articles ou qu’un flux rencontre un problème. Uniquement lorsque Curio est en arrière-plan.',
    'settings.notify.newArticles': 'Nouveaux articles',
    'settings.notify.errors': 'Erreurs d’actualisation',
    'settings.notify.feedDead': 'Un flux est mort',
    'settings.notify.quietStart': 'Heures silencieuses de',
    'settings.notify.quietEnd': 'Heures silencieuses à',
    'settings.notify.quietHours.hint':
        'Aucune notification pendant cette plage. Laissez vide pour désactiver.',
    'settings.notify.blocked': 'Les notifications sont bloquées dans les Réglages Système.',
    'settings.section.advanced': 'Avancé',
    'settings.refreshInterval': 'Intervalle d’actualisation',
    'settings.refreshInterval.hint':
        'Fréquence à laquelle Curio vérifie les nouveaux articles de vos flux.',
    'settings.refreshInterval.off': 'Désactivé',
    'settings.refreshInterval.m15': 'Toutes les 15 minutes',
    'settings.refreshInterval.m30': 'Toutes les 30 minutes',
    'settings.refreshInterval.h1': 'Toutes les heures',
    'settings.refreshInterval.h3': 'Toutes les 3 heures',
    'settings.refreshOnLaunch': 'Actualiser au démarrage',
    'settings.refreshOnLaunch.hint': 'Vérifier tous les flux une fois au démarrage de Curio.',
    'settings.updates.version': 'Version {version}',
    'settings.updates.check': 'Rechercher des mises à jour',
    'settings.updates.checking': 'Recherche…',
    'settings.updates.upToDate': 'Curio est à jour.',
    'settings.updates.available': 'Mise à jour disponible : {version}',
    'settings.updates.install': 'Installer et redémarrer',
    'settings.updates.installing': 'Téléchargement… {percent}%',
    'settings.updates.error': 'Impossible de rechercher des mises à jour.',
    'settings.updates.autoCheck': 'Rechercher les mises à jour automatiquement',
    'settings.updates.autoInstall': 'Installer les mises à jour automatiquement',
    'settings.updates.autoInstall.hint':
        'Télécharge et installe les mises à jour en arrière-plan, puis redémarre.',
    'updates.available': 'Curio {version} est disponible — ouvrez les Réglages pour installer.',
    'settings.comingSoon': 'Bientôt disponible dans une prochaine mise à jour.',
    'settings.customThemes': 'Thèmes personnalisés',
    'settings.customThemes.hint':
        'L’édition de vos propres thèmes arrivera dans une prochaine mise à jour.',
    'settings.homeYoutube': 'Disposition d’accueil YouTube',
    'settings.homeYoutube.hint':
        'Afficher les flux YouTube sous forme de grille de vidéos plutôt qu’en lignes compactes.',
    'settings.homeReddit': 'Disposition d’accueil Reddit',
    'settings.homeReddit.hint':
        'Afficher les flux Reddit sous forme de fil de publications plutôt qu’en lignes compactes.',
    'settings.allowRemoteFavicon': 'Autoriser les favicons distants',
    'settings.redditApi.title': 'API Reddit',
    'settings.redditApi.hint':
        'Facultatif. Avec votre propre app Reddit gratuite, les publications complètes se chargent à ~100 requêtes/min au lieu de ~10. Le secret est stocké dans le trousseau du système et ne quitte cet appareil que vers Reddit.',
    'settings.redditApi.status.on': 'Connecté — client id {clientId}',
    'settings.redditApi.status.off': 'Non connecté — endpoint public plus lent',
    'settings.redditApi.clientId': 'Client id',
    'settings.redditApi.clientSecret': 'Client secret',
    'settings.redditApi.save': 'Enregistrer',
    'settings.redditApi.remove': 'Supprimer les identifiants',
    'settings.redditApi.saved': 'Identifiants de l’API Reddit enregistrés dans le trousseau',
    'settings.redditApi.removed': 'Identifiants de l’API Reddit supprimés',
    'settings.redditApi.create': 'Créer une app gratuite sur reddit.com/prefs/apps',
    'settings.allowRemoteFavicon.hint':
        'Lorsqu’un site ne déclare aucun favicon, utiliser le service de favicons de Google. Désactivé par défaut : cela indique à Google quels sites vous prévisualisez (voir PRIVACY.md).',
    'settings.section.appearance': 'Apparence',
    'settings.language': 'Langue',
    'settings.language.hint':
        'La langue de l’interface. Le texte non traduit s’affiche en anglais.',
    'settings.section.reading': 'Lecture',
    'settings.markOnScroll': 'Marquer comme lu au défilement',
    'settings.markOnScroll.hint':
        'À mesure que les articles défilent hors de la liste, les marquer automatiquement comme lus. Désactivé par défaut — votre nombre de non lus ne change que si vous le décidez.',
    'settings.density': 'Densité de la liste',
    'settings.density.hint':
        'Confortable affiche une ligne d’aperçu et une vignette sur chaque ligne ; compacte affiche plus de lignes à l’écran.',
    'settings.density.comfortable': 'Confortable',
    'settings.density.compact': 'Compacte',
    'settings.section.media': 'Médias',
    'settings.media.prefetch': 'Charger les médias distants',
    'settings.media.prefetch.hint':
        'Récupérer les miniatures YouTube et les images intégrées aux publications via le cache local. Désactivé par défaut — rien ne se charge depuis le réseau tant que vous n’activez pas cette option.',
    'settings.section.destinations': 'Destinations',
    'settings.section.data': 'Données',
    'settings.section.diagnostics': 'Diagnostics',
    'settings.theme': 'Thème',
    'settings.typography': 'Typographie de lecture',
    'settings.about.version': 'Version {version}',
    'settings.about.profile': 'Profil : {path}',
    'settings.about.schema': 'Schéma de base de données v{version}',
    'theme.system': 'Système',
    'theme.select': 'Utiliser le thème {name}',
    'theme.active': 'Thème actif : {name}',
    'theme.import': 'Importer un thème…',
    'theme.export': 'Exporter en YAML…',
    'theme.delete': 'Supprimer',
    'theme.customActions': 'Actions du thème',
    'theme.customEmpty':
        'Aucun thème personnalisé pour l’instant — importez un thème .yaml pour en ajouter un.',
    'theme.imported': 'Thème « {name} » importé',
    'theme.exported': 'Thème exporté',
    'theme.importFailed': 'Échec de l’import : {error}',
    'theme.contrastWarning': 'Contraste faible — {pairs}',
    'theme.renamedOnImport': 'Importé sous « {id} » (cet id était pris)',
    'opml.title': 'Importer / exporter les abonnements',
    'opml.import': 'Importer un OPML…',
    'opml.export': 'Exporter un OPML…',
    'opml.importing': 'Importation…',
    'opml.exporting': 'Exportation…',
    'opml.imported':
        '{added, plural, one {# flux importé} other {# flux importés}}, {skipped, plural, one {# ignoré} other {# ignorés}}',
    'opml.exported': 'Abonnements exportés',
    'opml.cancelled': 'Annulé',
    'opml.hint':
        'Les fichiers sont choisis via une boîte de dialogue native — Curio ne voit jamais un chemin que vous n’avez pas sélectionné.',
    'import.title': 'Importer les abonnements et les articles enregistrés',
    'import.format': 'Format',
    'import.source.opml': 'OPML — abonnements',
    'import.source.pocket_csv': 'Pocket — CSV',
    'import.source.instapaper_csv': 'Instapaper — CSV',
    'import.source.readwise_csv': 'Readwise Reader — CSV',
    'import.run': 'Importer…',
    'import.running': 'Importation…',
    'import.done':
        '{feeds, plural, one {# flux} other {# flux}} et {articles, plural, one {# article} other {# articles}} importés, {skipped, plural, one {# ignoré} other {# ignorés}}',
    'import.hint':
        'Les fichiers OPML importent les abonnements aux flux ; les CSV de Pocket, Instapaper et Readwise importent les articles enregistrés comme éléments à lire plus tard, avec leurs étiquettes.',
    'doctor.title': 'Diagnostic de la base de données',
    'doctor.integrity': 'Vérifier l’intégrité',
    'doctor.ftsIntegrity': 'Vérifier l’index de recherche',
    'doctor.vacuum': 'Compacter la base de données',
    'doctor.sweep': 'Purger les anciens journaux d’événements',
    'doctor.diagnostics': 'Copier les diagnostics…',
    'doctor.running': 'En cours…',
    'doctor.passed': '{check} réussie',
    'doctor.vacuumed': 'Base de données compactée',
    'doctor.swept': '{count} anciens fichiers journaux supprimés',
    'doctor.diagnosticsReady': 'Ensemble de diagnostics écrit dans {path}',
    'doctor.stats.articles': '{count, plural, one {# article} other {# articles}}',
    'doctor.stats.db': 'Base de données : {path}',
    'doctor.check.integrity': 'Vérification de l’intégrité',
    'doctor.check.fts': 'Vérification de l’index de recherche',
    'feedHealth.title': 'État des flux',
    'feedHealth.status': 'État',
    'feedHealth.pause': 'Suspendre',
    'feedHealth.resume': 'Reprendre',
    'feedHealth.revive': 'Réactiver',
    'feedHealth.recent': 'Récupérations récentes',
    'feedHealth.none': 'Aucune récupération enregistrée pour l’instant',
    'feedHealth.new': '{count, plural, one {# nouveau} other {# nouveaux}}',
    'feedHealth.open': 'État du flux {name}',
    'feedHealth.badge.active': 'Actif',
    'feedHealth.badge.paused': 'Suspendu',
    'feedHealth.badge.dead': 'Inactif',
    'feedHealth.fetch.ok': 'OK',
    'feedHealth.fetch.not_modified': 'Non modifié',
    'feedHealth.fetch.error': 'Erreur',
    'feedHealth.hint.forbidden':
        'Le site a refusé la requête (403). Certains sites bloquent les lecteurs de flux — réessayez plus tard ou vérifiez l’URL du flux.',
    'feedHealth.hint.rateLimited':
        'Le site limite le débit des requêtes (429). Curio espace ses tentatives — réessayez dans un moment.',
    'feedHealth.unsubscribe': 'Se désabonner',
    'feedHealth.unsubscribe.confirm':
        'Supprimer cet abonnement ? Les articles enregistrés sont conservés.',
    'feedHealth.unsubscribe.remove': 'Supprimer',
    'feedHealth.unsubscribe.cancel': 'Annuler',
    'feedName.label': 'Nom',
    'feedName.save': 'Renommer',
    'feedMarkRead.action': 'Tout marquer comme lu',
    'feedTags.label': 'Dossiers et étiquettes',
    'feedTags.placeholder': 'Tech/Bases de données, favoris',
    'feedTags.hint':
        'Séparés par des virgules. Utilisez / pour imbriquer dans des dossiers, p. ex. Tech/Bases de données.',
    'feedTags.save': 'Enregistrer',

    'editFeed.title': 'Modifier le flux',
    'editFeed.section.details': 'Détails',
    'editFeed.section.health': 'État',
    'editFeed.notify': 'Me notifier pour ce flux',
    'editFeed.fullText': 'Récupérer le texte intégral',
    'editFeed.fullText.hint':
        'Charge la page web de chaque nouvel article et conserve le texte intégral extrait au lieu de l’extrait du flux.',
    'editFeed.url.label': 'URL du flux',
    'editFeed.url.copy': 'Copier',
    'editFeed.url.copied': 'URL du flux copiée',
    'editFeed.url.open': 'Ouvrir',
    'editFeed.site.label': 'URL du site',
    'editFeed.site.placeholder': 'https://example.com',
    'editFeed.description.label': 'Description',
    'editFeed.details.save': 'Enregistrer',
    'tagEditor.placeholder': 'Ajouter une étiquette…',
    'tagEditor.remove': 'Retirer {tag}',
    'refresh.announce.start': 'Actualisation des flux',
    'refresh.announce.done':
        'Actualisation terminée : {count, plural, one {# nouvel article} other {# nouveaux articles}} dans {feeds, plural, one {# flux} other {# flux}}',
    'refresh.inProgress': 'Actualisation… {done} sur {total}',
    'pane.sidebar.resize': 'Redimensionner la barre latérale',
    'pane.list.resize': 'Redimensionner la liste des articles',
    'toast.dismiss': 'Ignorer',
    'shortcut.category.navigation': 'Navigation',
    'shortcut.category.actions': 'Actions',
    'shortcut.category.views': 'Vues',
    'shortcut.category.app': 'Application',
    'shortcut.nextArticle': 'Article suivant',
    'shortcut.previousArticle': 'Article précédent',
    'shortcut.nextUnread': 'Prochain non lu (tous flux confondus)',
    'shortcut.openArticle': 'Ouvrir l’article dans le navigateur',
    'shortcut.toggleStar': 'Ajouter / retirer des favoris',
    'shortcut.toggleReadLater': 'Ajouter à / retirer de la liste à lire',
    'shortcut.toggleRead': 'Marquer comme lu / non lu',
    'shortcut.promote': 'Enregistrer l’article dans une destination',
    'shortcut.refreshFeed': 'Actualiser le flux sélectionné',
    'shortcut.refreshAll': 'Actualiser tous les flux',
    'shortcut.search': 'Rechercher des articles',
    'shortcut.viewAll': 'Aller à tous les articles',
    'shortcut.viewStarred': 'Aller aux favoris',
    'shortcut.viewReadLater': 'Aller à la liste à lire',
    'shortcut.viewArchived': 'Aller aux archives',
    'shortcut.viewFeeds': 'Aller à la liste des flux',
    'shortcut.help': 'Aide sur les raccourcis clavier',
    'shortcut.addFeed': 'Ajouter un flux',
    'shortcut.settings': 'Ouvrir les paramètres',
};
