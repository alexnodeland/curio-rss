...
| `apps/desktop` | Tauri 2 + Svelte 5 (runes) reader — the surface users judge | **Shipped — v0.2.0** (macOS universal DMG; `brew install --cask …/curio`). The Phase-4 reader plus a v0.2 polish round: context menus, tabbed settings, background refresh, notifications, source presets, a native menu bar, and an in-app updater |
...
flowchart TB
  subgraph heads["Heads (thin adapters)"]
    ...
    DESK["apps/desktop (shipped v0.2.0)<br/>Tauri 2 + Svelte 5 runes<br/>tauri-specta generated IPC"]
...
**`apps/desktop` (shipped — v0.2.0).** Deliberately thin: dozens of `#[tauri::command]` wrappers
