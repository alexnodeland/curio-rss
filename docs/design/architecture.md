...
| `apps/desktop` | Tauri 2 + Svelte 5 (runes) reader — the surface users judge | **Phase 4, in progress** — `curio-desktop` is in the workspace |
...
flowchart TB
  subgraph heads["Heads (thin adapters)"]
    ...
    DESK["apps/desktop (Phase 4)<br/>Tauri 2 + Svelte 5 runes<br/>tauri-specta generated IPC"]
...
**`apps/desktop` (Phase 4 head, in progress).** Deliberately thin: ~45 `#[tauri::command]` wrappers
