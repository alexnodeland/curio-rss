...
| `apps/desktop` | Tauri 2 + Svelte 5 (runes) reader — the surface users judge | later head |
...
`curio-core`'s dependency tree, and the CLI head's headless build breaks CI the day the
core stops compiling without a webview. This is what makes both futures — a server head
...
flowchart TB
  subgraph heads["Heads (thin adapters)"]
    ...
    DESK["apps/desktop (later)<br/>Tauri 2 + Svelte 5 runes<br/>tauri-specta generated IPC"]
...
- Streaming-enforced size caps, connect/total timeouts, shared pooled client, honest
  User-Agent.
...
**`apps/desktop` (later head).** Deliberately thin: ~40 `#[tauri::command]` wrappers
